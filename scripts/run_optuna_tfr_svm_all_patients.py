...
Запуск из каталога NeuronDeCo:
    ...
    cd /path/to/NeuronDeCo && python scripts/run_optuna_tfr_svm_all_patients.py \\
...
from lib.training.svm_epochs import (  # noqa: E402
    eval_one_epoch_svm_f1_macro,
    train_one_epoch_svm,
)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=float,
        default=1.1,
        help="Up-weight for cumulative loss metric (same as transformer script).",
    )
    p.add_argument(
...
def main() -> None:
    ...
    for subject_id in subjects:
        ...
        try:
            ...
            _n, _c, _f, _t_bins = X.shape
            num_classes = int(np.unique(y).shape[0])

            study_db = out_dir / f"{tfr_path.stem}_tfr_svm.db"
            ...
            study = optuna.create_study(
                ...
                sampler=sampler,
                storage=storage_url,
                study_name=f"{tfr_path.stem}_tfr_svm",
                load_if_exists=True,
            )
            ...
            objective = make_objective_engine(
                ...
                run_fold_fn=run_fold_fn_factory(
                    ...
                    train_one_epoch=train_one_epoch_svm,
                    eval_one_epoch_f1_macro=eval_one_epoch_svm_f1_macro,
                    loss_metric=loss_metric,
                ),
                aggregate_mode=args.cv_aggregate,
                params_fn=params_fn_factory_tfr_svm(
            ...
            err_txt = out_dir / f"tfr_{subject_id}_tfr_svm.error.txt"
            try:
    ...
    save_json(out_dir / "_TFR_SVM_RUN_REPORT.json", report)
    log(f"Report: {out_dir / '_TFR_SVM_RUN_REPORT.json'}")
